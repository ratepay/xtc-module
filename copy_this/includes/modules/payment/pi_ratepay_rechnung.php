...
class pi_ratepay_rechnung {
    ...
    /**
     * This constructor set's all properties for the pi_ratepay_rechnung object
     */
    function pi_ratepay_rechnung() {
        global $order;
        //Begin default
        $this->code = 'pi_ratepay_rechnung';
        $this->title = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_TEXT . "(1.0.0)";
        $this->public_title = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_TEXT_TITLE;
        $this->description = utf8_decode(MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_TEXT_DESCRIPTION);
        $this->enabled = ((MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_STATUS == 'True') ? true : false);
        //End default
        //Begin custom
        $this->profileId = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PROFILE_ID;
        $this->securityCode = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SECURITY_CODE;
        $this->sandbox = ((MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SANDBOX == 'True') ? true : false);
        $this->logs = ((MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_LOGS == 'True') ? true : false);
        $this->gtcURL = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_GTC;
        $this->privacyURL = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PRIVACY;
        $this->merchantPrivacyURL = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_PRIVACY;
        $this->merchantName = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_NAME;
        $this->bankName = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_BANK_NAME;
        $this->sortCode = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_CODE;
        $this->accountNr = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ACCOUNT_NR;
        $this->swift = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SWIFT;
        $this->iban = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_IBAN;
        $this->email = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_EMAIL;
        $this->extraField = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_EXTRA_FIELD;
        $this->min = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MIN;
        $this->max = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MAX;

        $this->owner = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_OWNER;
        $this->hr = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_HR;
        $this->court = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_COURT;
        $this->fon = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FON;
        $this->fax = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FAX;
        $this->street = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_STREET;
        $this->plz = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_PLZ;
        $this->piCouponIncTax = MODULE_ORDER_TOTAL_COUPON_INC_TAX;
        $this->piReCalculateTax = MODULE_ORDER_TOTAL_COUPON_CALC_TAX;
        //End custom

        $this->sort_order = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_ORDER;

        if ((int) MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ORDER_STATUS_ID > 0) {
            $this->order_status = MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ORDER_STATUS_ID;
        }
        $this->check();
        if (is_object($order)) {
            $this->update_status();
    ...
    function update_status() {
        global $order;
        if (($this->enabled == true) && ((int) MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ZONE > 0)) {
            $check_flag = false;
            $check_query = xtc_db_query("SELECT zone_id from "
                    . TABLE_ZONES_TO_GEO_ZONES . " WHERE geo_zone_id = '"
                    . MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ZONE . "' and zone_country_id = '"
    ...
    /**
     * This function checks whether to display RatePAY Rechnung or not
     *
     * @return boolean
     */
    function selection() {
        ...
        if ($customerXTC['customers_telephone'] == '') {
            $fieldsBool = true;
            $fields[] = array('title' => 'Telefon', 'field' => xtc_draw_input_field('pi_phone', ''));
        }

        if ($customerXTC['customers_dob'] == '0000-00-00 00:00:00') {
            $fieldsBool = true;
            $fields[] = array('title' => 'Geburtstag', 'field' => xtc_draw_input_field('pi_birthdate', '') . " " . PI_RATEPAY_RECHNUNG_VIEW_PAYMENT_BIRTHDATE_FORMAT);
        }

        if (($order->customer['company'] != '' || $order->billing['company'] != '') && $customerXTC['customers_vat_id'] == '') {
            $fieldsBool = true;
            $fields[] = array('title' => PI_RATEPAY_RECHNUNG_VIEW_PAYMENT_VATID, 'field' => xtc_draw_input_field('pi_vatid', ''));
        }

        if ($order->customer['company'] == '' && $order->billing['company'] == '' && $customerXTC['customers_vat_id'] != '') {
            $fieldsBool = true;
            $fields[] = array('title' => PI_RATEPAY_RECHNUNG_VIEW_PAYMENT_COMPANY, 'field' => xtc_draw_input_field('pi_company', ''));
        }

        if ($fieldsBool) {
            $display = array('id' => $this->code, 'module' => xtc_image(DIR_WS_IMAGES . '/ratepay_rechnung_rgb.png'), 'fields' => $fields);
        } else {
            $display = array('id' => $this->code, 'module' => xtc_image(DIR_WS_IMAGES . '/ratepay_rechnung_rgb.png'));
        }
    ...
    function verifyAge($dateStr) {
        ...
        if ($stampBirth > $stampToday) {
            $errorStr = urlencode(PI_RATEPAY_RECHNUNG_ERROR_AGE);
    ...
    function pre_confirmation_check() {
        global $order;
        if (isset($_SESSION['pi']['confirm']) && $_SESSION['pi']['confirm'] == true) {
            unset($_SESSION['pi']['confirm']);
            ...
            if ($response) {
                if ((string) $response->head->processing->status->attributes()->code == "OK" && (string) $response->head->processing->result->attributes()->code == "350") {
                    ...
                    $errorStr = urlencode(PI_RATEPAY_RECHNUNG_ERROR_GATEWAY);
                    xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $errorStr, 'SSL'));
                }
            } else {
                $_SESSION['disable'] = true;
                $errorStr = urlencode(PI_RATEPAY_RECHNUNG_ERROR);
                xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $errorStr, 'SSL'));
            }
        } else {
            $successFon = false;
            ...
            if (isset($_POST['pi_phone']) && isset($_POST['pi_birthdate'])) {
                ...
                if ($_POST['pi_phone'] != '') {
                    $successFon = true;
                    $customerId = $_SESSION['customer_id'];
                    xtc_db_query("update " . TABLE_CUSTOMERS . " set customers_telephone = '" . xtc_db_prepare_input($_POST['pi_phone']) . "' WHERE customers_id ='" . xtc_db_input($customerId) . "' ");
                }

                $dob = xtc_db_prepare_input($_POST['pi_birthdate']);
            ...
            } else if (isset($_POST['pi_birthdate'])) {
                $inputNeededBirthdate = true;
            ...
            if (($customerXTC['customers_vat_id'] == '' && ($order->customer['company'] != '' || $order->billing['company'] != '')) || ($customerXTC['customers_vat_id'] != '' && ($order->customer['company'] == '' || $order->billing['company'] == ''))) {
                ...
                if ($customerXTC['customers_vat_id'] == '') {
                    if ($_POST['pi_vatid'] != '') {
                        $_SESSION['pi']['vatid'] = $_POST['pi_vatid'];
                        ...
                        $errorStr = urlencode(PI_RATEPAY_RECHNUNG_ERROR_VATID_ERROR);
                        xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $errorStr, 'SSL'));
                    }
                }
                ...
                if ($order->customer['company'] == '') {
                    if ($_POST['pi_company'] != '') {
                        $_SESSION['pi']['company'] = $_POST['pi_company'];
                        $order->customer['company'] = $_POST['pi_company'];
                        ...
                        $errorStr = urlencode(PI_RATEPAY_RECHNUNG_ERROR_COMPANY_ERROR);
                        xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $errorStr, 'SSL'));
                    }
                }
            }
            ...
            if ($inputNeededFon == true && $inputNeededBirthdate == true) {
                ...
                if ($successFon == true && $successDate == true) {
                    xtc_redirect(xtc_href_link("pi_ratepay_rechnung_checkout_terms.php", '', 'SSL'));
                } else {
                    if ($successFon == false && $successDate == false) {
                        $errorStr = urlencode(PI_RATEPAY_RECHNUNG_ERROR_PHONE_AND_BIRTH);
                        xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $errorStr, 'SSL'));
                    } else if ($successDate == false) {
                        $errorStr = urlencode(PI_RATEPAY_RECHNUNG_ERROR_BIRTH);
                        xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $errorStr, 'SSL'));
                    } else if ($successFon == false) {
                        $errorStr = urlencode(PI_RATEPAY_RECHNUNG_ERROR_PHONE);
                        xtc_redirect(xtc_href_link(FILENAME_CHECKOUT_PAYMENT, 'error_message=' . $errorStr, 'SSL'));
                    }
                }
    ...
    function after_process() {
        ...
        if ($response) {
            ...
            if ((string) $response->head->processing->status->attributes()->code == "OK" && (string) $response->head->processing->result->attributes()->code == "400") {
                ...
                $sql = "INSERT INTO pi_ratepay_rechnung_orders (order_number, transaction_id, transaction_short_id, descriptor)
                ...
                for ($i = 0; $i <= sizeof($neworder->products); $i++) {
                    ...
                    if ($price > 0) {
                        $sql = "INSERT INTO pi_ratepay_rechnung_orderdetails (order_number,article_number, real_article_number, article_name,ordered,article_netUnitPrice)
    ...
    function check() {
        if (!isset($this->_check)) {
            $check_query = xtc_db_query("SELECT configuration_value FROM " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_STATUS'");
     ...
     */
    function install() {
        $check_query = xtc_db_query("SHOW TABLES LIKE 'pi_ratepay_rechnung_orders'");
        if (xtc_db_num_rows($check_query) == 0) {
            xtc_db_query(
                    "CREATE TABLE `pi_ratepay_rechnung_orders`(
        ...
        $check_query = xtc_db_query("SHOW TABLES LIKE 'pi_ratepay_rechnung_orderdetails'");
        if (xtc_db_num_rows($check_query) == 0) {
            xtc_db_query(
                    "CREATE TABLE `pi_ratepay_rechnung_orderdetails` (
        ...
        $check_query = xtc_db_query("SHOW TABLES LIKE 'pi_ratepay_rechnung_history'");
        if (xtc_db_num_rows($check_query) == 0) {
            xtc_db_query(
                    "CREATE TABLE `pi_ratepay_rechnung_history` (
						  `id` INT NOT NULL AUTO_INCREMENT,
						  `order_number` VARCHAR( 255 ) NOT NULL ,
						  `article_number` VARCHAR( 255 ) NOT NULL ,
        ...
        if (xtc_db_num_rows($check_query) == 0) {
            ...
            xtc_db_query("ALTER TABLE admin_access ADD pi_ratepay_print_order INT(1) NOT NULL DEFAULT '0'");
            ...
            xtc_db_query("UPDATE admin_access SET pi_ratepay_print_order = '1' WHERE customers_id='1' OR customers_id= 'groups'");
        ...
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_STATUS', 'True', '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PROFILE_ID', '', '6', '3', NOW())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SECURITY_CODE', '', '6', '3', NOW())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SANDBOX', 'False', '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_LOGS', 'False', '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_GTC', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PRIVACY', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_PRIVACY', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_NAME', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_BANK_NAME', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_CODE', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ACCOUNT_NR', '', '6', '0', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SWIFT', '', '6', '0', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_IBAN', '', '6', '0', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MAX', '1500', '6', '0', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MIN', '0', '6', '0', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_EXTRA_FIELD', '', '6', '0', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_ORDER', '0', '6', '0', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, use_function, set_function, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ZONE', '0', '6', '2', 'xtc_get_zone_class_title', 'xtc_cfg_pull_down_zone_classes(', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, set_function, use_function, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ORDER_STATUS_ID', '0', '6', '0', 'xtc_cfg_pull_down_order_statuses(', 'xtc_get_order_status_name', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ALLOWED', '', '6', '0', now())");

        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_OWNER', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_HR', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FON', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FAX', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_PLZ', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_STREET', '', '6', '3', now())");
        xtc_db_query("INSERT INTO " . TABLE_CONFIGURATION . " (configuration_key, configuration_value, configuration_group_id, sort_order, date_added) VALUES ('MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_COURT', '', '6', '3', now())");
    }

    /**
     * Removes all RatePAY Rechnung DB Entries
     */
    function remove() {
        xtc_db_query("DELETE FROM " . TABLE_CONFIGURATION . " WHERE configuration_key in ('" . implode("', '", $this->keys()) . "')");
        xtc_db_query("DROP TABLE `pi_ratepay_log`, `pi_ratepay_rechnung_history`, `pi_ratepay_rechnung_orderdetails`, `pi_ratepay_rechnung_orders`");
    }

    /**
     * Setting all the RatePAY Rechnung Keys for Configuration
     *
     * @return array
     */
    function keys() {
        return array(
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_STATUS',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PROFILE_ID',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SECURITY_CODE',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MIN',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MAX',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SANDBOX',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_LOGS',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_GTC',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_PRIVACY',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_PRIVACY',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_OWNER',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FON',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_FAX',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_STREET',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_PLZ',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_COURT',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SHOP_HR',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_MERCHANT_NAME',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_BANK_NAME',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_CODE',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ACCOUNT_NR',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SWIFT',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_IBAN',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_EXTRA_FIELD',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_SORT_ORDER',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ALLOWED',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ZONE',
            'MODULE_PAYMENT_PI_RATEPAY_RECHNUNG_ORDER_STATUS_ID'
        );
    }
    ...
    function paymentRequest($order, $xtPrice) {
        ...
        $payment->addChild('amount', number_format(round($total, $xtPrice->get_decimal_places($currency)), 2, ".", ""));
    ...
    function piRatepayLog($orderId, $transactionId, $payment_type, $payment_subtype, $request, $response = false, $first_name = '', $last_name = '') {
        ...
        if ($logging == true) {
            ...
            $sql = "INSERT INTO pi_ratepay_log (order_number, transaction_id, payment_method, payment_type,  payment_subtype, result, request, response, result_code, first_name, last_name, reason) VALUES ('" . xtc_db_input($orderId) . "', '" . xtc_db_input($transactionId) . "', 'INVOICE','" . xtc_db_input($payment_type) . "', '" . xtc_db_input($payment_subtype) . "', '" . xtc_db_input($result) . "','" . xtc_db_input($requestXML) . "','" . xtc_db_input($responseXML) . "','" . xtc_db_input($resultCode) . "','" . xtc_db_input($first_name) . "','" . xtc_db_input($last_name) . "','" . xtc_db_input($reasonText) . "')";
    ...
    function getShippingTaxAmount($order) {
        ...
        $shippingTaxAmount = $order->info['shipping_cost'] * ($this->getShippingTaxRate($order) / 100);
