...
class PiRatepayRateCalcBase
{
    ...
    public function PiRatepayRateCalcBase()
    {
        ...
        $this->request_profile_id = $picalcdata->getProfileId();

        if ($picalcdata->getSecurityCode() == '') {
            $this->request_security_code = $picalcdata->getSecurityCodeHashed();
        } else {
            $this->request_security_code = md5($picalcdata->getSecurityCode());
        }

        $this->request_transaction_id = $picalcdata->getTransactionId();
        $this->request_transaction_short_id = $picalcdata->getTransactionShortId();
        $this->request_order_id = $picalcdata->getOrderId();
        $this->request_merchant_consumer_id = $picalcdata->getMerchantConsumerId();
        $this->request_merchant_consumer_classification = $picalcdata->getMerchantConsumerClassification();
        $this->request_amount = $picalcdata->getAmount();
        $this->request_live = $picalcdata->isLive();

        $this->language = $picalcdata->getLanguage();

        $this->request_operation = '';
        $this->request_operation_subtype = '';
        $this->request_calculation_value = '';
        $this->request_due_date = '';
        $this->request_payment_firstday = $picalcdata->getPaymentFirstdayConfig();
        $this->request_interest_rate = $picalcdata->getInterestRate();
    ...
    {
        $picalcdata = new PiRatepayRateCalcData();
        $picalcdata->setData($total_amount, $amount, $interest_rate, $interest_amount, $service_charge, $annual_percentage_rate, $monthly_debit_interest, $number_of_rates, $rate, $last_rate, $payment_firstday);
    }
